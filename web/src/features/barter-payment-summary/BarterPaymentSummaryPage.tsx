...
/* ----------------------------- primitives ---------------------------- */
function EditBtn({ left, top }: { left: number; top: number }) {
  return (
    <span
      className="absolute flex h-[20px] w-[20px] items-center justify-center rounded-[10px] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.10)]"
      style={{ left, top }}
    >
...
function TableRow({ nameLeft, top, name, delRight, editLeft, onClick }: { nameLeft: number; top: number; name: string; delRight: number; editLeft: number; onClick?: () => void }) {
  return (
    <>
      ...
      <EditBtn left={editLeft} top={top + 3} />
    </>
  );
}

function AgencyFeeRow({ labelLeft, top, pillLeft, addedLeft, green, amountRight }: { labelLeft: number; top: number; pillLeft: number; addedLeft: number; green: string; amountRight: number }) {
  return (
    <>
      ...
      {/* Added pill */}
      <div className="absolute h-[22px] rounded-[24px] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.08)]" style={{ left: pillLeft, top: top + 1, width: 117 }} />
      <span className="absolute text-[12px] font-normal leading-none text-black" style={{ left: addedLeft, top: top + 6 }}>Added</span>
      <span className="absolute text-[12px] font-medium leading-none text-[#3DBB6C] underline" style={{ left: addedLeft + 41, top: top + 6 }}>{green}</span>
      <EditBtn left={pillLeft + 109} top={top + 2} />
      {/* amount */}
...
export default function BarterPaymentSummaryPage() {
  ...
  return (
    <>
      ...
      <span onClick={() => navigate("/invoices")} className="absolute cursor-pointer text-[24px] font-normal leading-none text-black underline underline-offset-4" style={{ left: 600, top: 240 }}>{barterInvoice?.number ?? ""}</span>
      ...
      {/* internal creators */}
      {internal.map((c, i) => (
        <LeftRow key={c.id} nameTop={352 + i * 47} name={c.name} handle={c.handle} grad={INTERNAL_GRADS[i]} onClick={() => navigate("/creators/detail")} />
      ))}
      ...
      {/* external creators */}
      <LeftRow nameTop={557} name={external[0]?.name ?? ""} handle={external[0]?.handle ?? ""} grad={EXTERNAL_GRADS[0]} onClick={() => navigate("/creators/detail")} />
      <LeftRow nameTop={606.7} name={external[1]?.name ?? ""} handle={external[1]?.handle ?? ""} grad={EXTERNAL_GRADS[1]} onClick={() => navigate("/creators/detail")} />
      <LeftRow nameTop={656.4} name={external[2]?.name ?? ""} handle={external[2]?.handle ?? ""} grad={EXTERNAL_GRADS[2]} onClick={() => navigate("/creators/detail")} />
      ...
      <span className="absolute flex h-[35px] w-[35px] items-center justify-center rounded-full bg-white/50" style={{ left: 947, top: 367 }}>
      ...
      <TableRow nameLeft={711} top={536} name={internal[0]?.name ?? ""} delRight={182} editLeft={1263} onClick={() => navigate("/creators/detail")} />
      <TableRow nameLeft={711} top={569} name={internal[1]?.name ?? ""} delRight={182} editLeft={1263} onClick={() => navigate("/creators/detail")} />
      <TableRow nameLeft={711} top={602} name={internal[2]?.name ?? ""} delRight={182} editLeft={1263} onClick={() => navigate("/creators/detail")} />
      <AgencyFeeRow labelLeft={711} top={636} pillLeft={804} addedLeft={826} green="₹1000" amountRight={157} />
      ...
      <TableRow nameLeft={712} top={759} name={external[0]?.name ?? ""} delRight={181} editLeft={1264} onClick={() => navigate("/creators/detail")} />
      <TableRow nameLeft={712} top={792} name={external[1]?.name ?? ""} delRight={182} editLeft={1264} onClick={() => navigate("/creators/detail")} />
      <TableRow nameLeft={712} top={825} name={external[2]?.name ?? ""} delRight={182} editLeft={1264} onClick={() => navigate("/creators/detail")} />
      <AgencyFeeRow labelLeft={712} top={859} pillLeft={805} addedLeft={827} green="₹2000" amountRight={156} />
