 ...
 *
 * The Creators list in multi-select mode with the Sort bottom sheet open over
 * the 57% scrim. The sheet is shorter than the filter sheet — 375x356 pinned at
 * y=520, 32/32/0/0 corners — and its body is a plain 327-wide radio list: four
 * options on a 56pt step, separated by 1px hairlines. Single select, no Apply
 * button: tapping an option re-sorts the list underneath and dismisses.
 *
...
export default function CreatorsSortSheetScreen() {
   ...
   * server would apply for GET /creators?orderBy=.
   */
  ...
  return (
    <Screen height={FRAME_H} background={PAGE} scroll>
      ...
      {/* D / W / M window switch */}
      <Abs x={16} y={216} w={343} h={42} radius={16} bg={WHITE} />
      <Abs x={21} y={221} w={111} h={32} radius={12} bg={INK} />
      <Txt
        x={72.17} y={229} w={8.67} size={12} weight="bold" font="inter"
        color={INK_INVERT} lineHeight={16} align="center"
      >
        D
      </Txt>
      <Txt
        x={181.38} y={229} w={12.25} size={12} weight="semibold" font="inter"
        color={INK_MUTED} lineHeight={16} align="center"
      >
        W
      </Txt>
      <Txt
        x={292.96} y={229} w={11.08} size={12} weight="semibold" font="inter"
        color={INK_MUTED} lineHeight={16} align="center"
      >
        M
      </Txt>
      ...
      {/* Selection banner */}
      <Abs x={16} y={275} w={343} h={42} radius={16} bg={BANNER} />
      ...
      {/* Card stack — the real roster in the chosen order */}
          ...
          <Fragment key={o.key}>
            ...
            >
              ...
              <View
                style={[
                  styles.radio,
                  { borderColor: on ? RADIO_ON : HAIRLINE },
                ]}
              >
