...
function Chip({
  ...
  tint: { bg: string; line: string; ink: string };
  label: string;
}) {
  return (
...
export default function LeadAddedSuccess() {
  ...
  const chips: { key: string; icon: keyof typeof Feather.glyphMap; tint: typeof CHIP_TINT.deal; label: string }[] =
    [];
  if (lead?.channel) {
  ...
  const createAnother = () =>
    router.canGoBack() ? router.back() : router.push("/leads/leads-list");
  const viewCampaign = () => router.push("/campaigns/active-campaigns");
